...
- **e2e 等异步 UI（浮卡弹出/状态翻转/网络落库）一律 `waitForFunction` 等真实条件，禁定值 sleep**——并发 e2e 文件每多一个 Chromium 实例，既有 `waitForTimeout` 就会被 CPU 饿死成漂移红（v7.8.0 实锤：加第 4 个文件即挂 V545-2）。**三 bump 后必 sed 同步版本 pin**：断言散在约 10 个 unit 测试（`APP_VERSION`/versionCode/versionName/serverInfo/`Version X`），漏改必红；但 v770.test.js 等文件里的**历史版本注释不许一起 sed**；**数字 pin（去点 versionCode）正则必须限定断言上下文**——裸 `(?<!\d)800(?!\d)` 连 `800ms` 计时常量都会照改（v8.0.1 实锤误伤彩蛋窗口后回正）。
