...
- **防爆破**：同一设备对同一篇笔记连续输错口令 20 次（10 分钟窗口内），锁 10 分钟（锁住期间对的口令也进不去）——常用笔记建议用长口令
- **密文到底**：提醒、草稿、历史版本全部随正文同钥加密；离线时也只落密文，不落明文
- **两个等同口令的东西**：解锁二维码、以及存在本机的密钥文件/`mcp.json`——都别截图外传、别提交进仓库

### 笔记写入凭据（v10.0.0 起）与手工解除认领

**它解决什么**：在此之前，笔记接口只有"读需要口令"，写入是**完全不设防**的——任何人只要猜中你的笔记名，就能把自己编的一串密文覆盖上去（他读不懂内容，但能毁掉）。现在每篇笔记登记着一把写入凭据的哈希，写入必须带上能对上号的凭据，服务器才收。

**它怎么来的、要不要额外记**：不需要。凭据是**你解锁时那把密钥派生出来的**（域分离串带上了笔记名），所以"能解密"和"能写入"是同一件事。好处是：**一篇笔记只需被认领一次，之后所有设备通用**——换手机、重装浏览器、用配对二维码，只要笔记名和口令对，开箱就能读写，不必重复认领；MCP 工具因为本地存着口令，也自动带着同一把凭据。改口令也不会失效：那一枪同时完成"写新密文 + 换绑凭据"。

**它挡不住什么（诚实的边界）**：一篇笔记在你**第一次用 v10.0.0 打开它之前**处于未认领状态，此时知道笔记名的人可以抢先登记他自己的凭据，把你挡在写入之外（注意：他本来就能直接覆盖你的笔记，所以抢注并没有给他新的破坏力，只是换了一种捣乱方式）。因此**发版当天请把全部笔记在 PC 和手机上各打开一遍**——首次解锁即自动认领，这个窗口就永久关上了。

**真被抢注了怎么恢复（一步，你自己能做）**：登录服务器，把那篇笔记档里的 `wkHash` 字段删掉，它就回到"未认领"状态，你随后一次写入会重新登记成你自己的凭据；正文、版本、盐、提醒全都不受影响。

```powershell
# 服务器上执行，把 <笔记名> 换成实际名字
powershell -NoProfile -Command "$p='C:\Services\NoteSync\data\notes\<笔记名>.json'; $o=Get-Content $p -Raw | ConvertFrom-Json; $o.PSObject.Properties.Remove('wkHash'); [IO.File]::WriteAllText($p+'.tmp', ($o|ConvertTo-Json -Compress)); Move-Item -Force ($p+'.tmp') $p"
```

两处细节别改回简单写法：`Set-Content -Encoding utf8` 在 Windows PowerShell 5.1 会写 **BOM**，服务端读档解析失败会静默当成空档、下一次打开就可能把真档覆写成空；就地写文件也有撕裂风险，所以走「写 .tmp → Move-Item -Force」原子替换（与 server.js 自身写法一致）。改完不用重启服务（每次请求实时读盘）。若该笔记正被某台设备编辑，先关掉那个页面再改，避免它下一秒又把自己的凭据写回去。

**服务端开关**：`data/wk-mode.txt` 里写 `off` / `new-only` / `full` 即可热切换（5 秒内生效，不用重启）。`off`=只登记不拦（发布初态）；`new-only`=只挡无凭据的新建；`full`=已认领的笔记拒绝一切无凭据写入。
...
| 版本 | 日期 | 摘要 |
|------|------|------|
| v10.0.3 | 2026-09-21 | 折叠列表语义改造（用户报障两症状同源）。①**折叠组不再被空行切断**：旧规则「[折叠] 往下折到第一个空行为止」让空行既当排版又当边界——展开态在标题与正文间留个空块，整段正文就掉出组、三角再也折不回去；收起态在标题上回车，把手被劈成「[折叠]前半句 + 隐藏后半句」，新打的字一并被吞进收起的组里（字没删，只是看不见）。新规则：一组从 `[折叠]` 那行往下，直到**下一个 `[折叠]`**、或 **`[/折叠]` 闭合锚**、或**笔记末尾**；空行照旧走缩进与左引导线，收起时一并藏起（＝你要求的"标题以下全归组"）。②**`[/折叠]` 闭合锚**：要"在折叠外面继续写"才需要它，系统**只在你要出来的那一刻自动补**（不按回车就一个字都不加，折叠写在笔记末尾的存量笔记完全不受影响）；主形态挂在组内最后一行行尾（`font-size:0`，编辑器与导出图零视觉痕迹、不新增行），整行只有锚时压成 0 高不占位；删 `[折叠]` 把手会联动清掉本组锚。③**把手行回车重写**：光标在标题行任意位置回车 = 「[折叠]」+光标前的字留在把手、光标后那段（末尾回车时为空）整体挪到整组之后＝组外，光标跟到它开头接着打字；组内正文与展开/收起态一律不动（替掉 v9.3.7 只挡"收起态+标题末尾"那一格的旧守卫）。④**护栏**：隐形锚不是停靠点——光标落进锚内或锚后即弹回锚之前（在那儿打字会把锚顶到行中间令整组散架）；独立行锚同样不许停靠。⑤**修导出图片里折叠正文没有缩进和左竖线**（同日报障）：基规则锁 `#editor` 作用域、导出离屏副本吃不到（v10.0.2 只补了 ▼ 漏了这条）；现导出副本逐字复刻缩进+引导线，`[/折叠]` 在出图里同样不露字，快慢两条渲染路径同源。⑥**存量语义变更（拍板项，非疏漏）**：老笔记里"标题下空一行再写正文"的形态，本版起该行归入折叠。纯网页层热更、无 Kotlin；OTA 即达 |
| v10.0.2 | 2026-09-20 | 导出图三合一：①页脚 tagline 改 16 句随机池（品牌原句「记录，自有回响」×2 权重）；②折叠列表出图不再显示裸 [折叠] 文字——导出副本挂属性门控作用域样式画 ▼ 恒定展开（编辑器本体 CSS 零改动），三角与标题间 7px 间隙（em 在 font-size:0 上归零故必须 px）；③tail 窄屏防裁：右对齐可折两行、仍放不下才 ellipsis（实测手机现状卡宽本就放得下，仅 288px 极端档触发）；④导出提速：空闲预取渲染组件+图片预解码+Chromium 触屏走 SVG 原生光栅（实测 5~14 倍，异常恒回退 html2canvas 老链路；顺带根修快路漏拆离屏卡的泄漏）。纯网页层热更、无 Kotlin |
| v10.0.1 | 2026-09-19 | 两报障双修：①导出图片右上角笔记名刻印**字母下半截被裁**——刻印 span 行高=字号（11px/1）配省略号承重的 overflow:hidden，盒子恰好字高，g/p/q/y/j 下伸部被裁掉；行高提到 1.6（省略号与 45% 限宽原样保留）。②「立即更新」经常报**下载已取消/失败**——根因是冷启动 6s 静默预下载与手动下载同 URL **互撤单**：原生桥旧规「同 url 撤旧单+删半成品+从零重下」，撞上的另一方立刻报「被取消」，删文件还会把没撞上的下载写成截断件；原生桥改「在跑单接管」（按远端 URL+落盘路径找回未完成单，直接交回其 id 继续轮询），预下载标记落盘 localStorage 并让位在手动下载，「重试下载」补上漏传的字节数（旧版重试退「>1MB 即复用」，截断坏件直通安装器＝packageInfo is null 死循环根因之一），失败提示同步上屏下载器 reason 码便于定位。核实修正：APK 下载自 v9.3.2 起已是境内 biji 域版本钉死直链（带 Range），GitHub 直连并非本版失败根因。含原生 UpdatePlugin：须 OTA 本版 APK；网页层随部署即时生效 |
| v10.0.0 | 2026-09-19 | **安全加固版**（六项合并一版）。①笔记接口补写入凭据：此前 `PUT /api/note/:id` 零鉴权，猜中笔记名即可覆盖写（读只拿密文、毁很容易）——现由解锁后的 AES 密钥本地派生凭据随写入带上，服务端只存哈希，零知识不变，语义即「能解密才能写入」，口令/解锁/配对流程一字未改；**未认领的笔记接受第一次凭据登记（=认领），此后凭据不符一律硬 403、不会自动降级**；真主认领前存在"抢注冻结写入"窗口，恢复只需在服务器删掉该档 `wkHash`（见「手工解除认领」），故发版当天请把全部笔记各打开一遍让认领完成；②改口令改为「同一次写入原子换绑」，避免换密钥后被自己的凭据闸锁死；③历史快照两处侧门接同一判据，并堵掉凭空造历史文件的磁盘填充面；④修复口令爆破锁双重失效（客户端上报不带 JSON 头被自家规则恒定 400，计数器从未加过），修好后同步放宽为错 20 次锁 10 分钟；⑤图片上传改服务端一次一签（免签名 preset 连 cloud 名一起印在页面源码=开放写入口），硬编码 preset 删除，图片内容维持明文；⑥渲染消毒：解密正文上屏前过一次白名单（名单从全仓实测来，折叠/提醒/链接的承重属性一律放行；href 里的 data: 一律拒，含 data:image/svg+xml 型载荷），且只在真删了东西时才改写字符串，不污染冲突检测的比对。另：关键路径空 catch 止血（密钥落盘/草稿/缓存/提醒草稿失败不再静默）、CI 新增前置测试闸（单测不绿不出包）、MCP 自带凭据、发版部署通道改用系统 ssh + 密钥（弃用不兼容的 paramiko 与明文密码文件）。**明确接受的风险（拍板项，非疏漏）**：密钥仍驻 localStorage 免密、CSP 仍放开内联脚本（单文件架构必然代价）、图片内容仍明文上云、笔记名不设最小长度。灰度：凭据强制开关三态，发布置 off，第 2 天只挡新建，第 14 天对**已认领**笔记全量强制（未认领存量继续放行，宁可少挡也不制造不可自愈的锁死）——门槛只能落在本版本（首个自带凭据的版）。凭据失败只挡写、不牵连读；**已认领 + 错凭据 = 硬 403 且不改写登记哈希**（撞几下也卸不掉防护）。含原生 versionCode/versionName：须 OTA 本版 APK；服务端与网页层随部署即时生效 |
| v9.5.8 | 2026-09-19 | 启动页收尾四件（用户拍板）：①slogan 定稿「落笔即心安」（此心安处是吾乡的人本状态，非保险话术"安心"），首页与幕布两处同词；②系统闪屏改纯纸底（甲案）——品牌只由幕布呈现一次，杜绝「系统环↔幕布环」双源位置跳动（空图必须显式给，缺省会回落 Launcher 图标）；③tank 基地与 bitcoin 创世块弃记事本纸片，改画布品牌图章（环+N 无尖小尺寸档、纯令牌、日夜皮肤自动同权）；④安装引导由单行提示条升级扫码配对同款浮卡（品牌角标+衬线标题+细金线+反相主按钮，「以后再说」只收本会话、× 才是不再提示）。含原生 strings/styles+退役 2 素材增 1 空图：原生项须 OTA 本版 APK；①③④网页层随本版部署即时生效 |
| v9.5.7 | 2026-09-19 | 启动页真机三坑修复（v9.5.6 上线后用户实拍回炉，选 A 案保留幕布）：①A12+ 系统闪屏把图标放大 288dp 套蒙版裁外圈 1/3，满幅环被切成"角括号"→新增中央 1/3 安全区素材 splash_icon_*（幕布仍用满幅 splash_logo_*，两套各司其职）；②CoordinatorLayout 默认子参数=WRAP_CONTENT+左上，幕布真机塌成左上窄条（字标折成「NoteS」、金线/slogan 被裁）→显式 MATCH_PARENT 全屏参数+单列 LinearLayout 整体居中+文字锁单行；③秒开时幕布 250ms 即掀、slogan 永不可见→双条件揭幕：页面落地≥250ms 且系统闪屏退场（App 首帧记时，不用会吞系统退场动画的 exit listener）≥450ms 取 max，15s 兜底与错误即掀不变。网页层零变化。含原生 MainActivity+styles+2 新 vector：须 OTA 本版 APK |
| v9.5.6 | 2026-09-19 | 启动页纸墨化（用户拍板方案 A2+slogan「落笔即安心」）：①正规启用「依赖已在却从未 install」的 androidx SplashScreen——冷启空窗由纸底+带箭尖品牌环系统闪屏承接，日夜按时间规则（与 shouldBeDark 逐字同 420/1140，不跟随系统）setTheme 选显式变体；②原生 A2 幕布（品牌环+衬线字标+金细线+slogan）盖住 WebView 加载期，主文档落地 250ms 淡出、15s 硬超时兜底、断网兜底页出现即掀幕；③网页首页 landing sub 由功能句换「落笔即安心」（功能语义仍由底部信任行承载）。含原生 MainActivity.java+res（server.js/Caddyfile 未动）：原生项须 OTA 本版 APK，网页层随本版部署即时生效 |
| v9.5.5 | 2026-09-19 | 启动「全白/全黑很久→加载中又挂半天」与「黑屏闪几下→白加载中→黑加载中」三批根治：网页层——head 首帧前按时间规则预置夜色灭白闪、原生冷启自动跳上提到 head（省 root 空跑 760KB 一整轮白屏）、#k= 配对与 2s 轮询补超时、同步先于提醒恢复、JS 顶层心跳供原生探活；服务端——主文档 ETag/304 且落到 App 链路（native 拦截器带条件请求、304 吐缓存，复访传输≈0；此前每次冷启全量重下 275KB）、SSE 上限检查修头序、Caddy 摘 no-store 放行 304；原生——首载 6s watchdog 重走带超时的三级兜底拦截器（半死 socket 白屏从无限封顶 ~15s）、WebView 背景按日夜预置、后台回来的无条件 reload 改「心跳探活只救真死页」（v9.5.4 的无脑 reload 正是闪烁制造者）、渲染进程二次死亡补提示不再静默闪退。（HTTP/2 因历史 SSE 降级未轻翻，留专项验证）含 Kotlin+server.js+Caddy：原生项须 OTA 到本版 APK；服务端/网页层随本版部署即时生效 |
| v9.5.4 | 2026-09-19 | 修「点立即安装报 packageInfo is null」+「启动全白/全黑、加载中挂半天」：①OTA 下载精确字节校验（JS 从 /api/latest 带 asset.size，原生复用/done 要求文件字节精确相等，坏/截断文件即删重下，杜绝坏 APK 被死循环复用）；②下载 URL 改版本固定名 /dl/vX.Y.Z.apk（不可变副本，根除下载中途 latest.apk 被覆盖致新旧混装），服务器只留最近 2 份；③WebView 渲染进程被 MIUI 查杀致全白/全黑→onRenderProcessGone 自动重建自愈（进程内一次防循环）；④后台超 10 分钟回前台主动 reload 兜「冻而未死」；⑤启动/解锁首拉 apiGet 加 8s 超时，半死 socket 下不再挂「加载中」，超时落既有离线缓存渲染。含 Kotlin 改动，须 OTA 到本版 APK 才吃到①③④；②⑤网页/服务端层即时生效 |
| v9.5.3 | 2026-09-19 | 更新弹窗「拉起」黑话清零：进行中按钮「拉起安装…」改「正在安装…」、授权后重试钮改「立即安装」、失败提示「安装页没拉起来」改「立即安装没成功」。纯文案、纯网页层热更、无 Kotlin 改动 |
| v9.5.2 | 2026-09-19 | 更新弹窗两处文案：安装页拉起后提示改「点「立即安装」升级，笔记和数据都会保留」，重试按钮「重新拉起安装」改「立即安装」。纯文案、纯网页层热更、无 Kotlin 改动 |
| v9.5.1 | 2026-09-19 | 修移动端三处「不该弹却弹/不收起键盘」：导出为图片钮（全排唯一漏收键盘的顶栏动作，点它后渲染期间键盘一直挂着）、主题切换钮（同款补收）、提醒面板保存写回正文后（无条件聚焦弹键盘，现收——桌面光标续输入语义不变）。收键盘判据并加强为「App 壳内强制收」，杜绝国产 WebView 把触屏误报成鼠标环境时永不生效的老洞；均带组字期守卫、桌面零影响。纯网页层热更、无 Kotlin 改动 |
| v9.5.0 | 2026-09-18 | 导出图片升级为极简书纸卡：外衬卡裱内衬两层、页头金题线+单行日期+笔记名等宽刻印、页脚顶栏真品牌标克隆+「来自 NoteSync」+tagline「记录，自有回响」，全图金色仅两锚点零新色，桌面出图宽封顶 640（手机照旧屏宽），日夜随当前主题；修移动端坦克等全屏彩蛋点屏开火整屏闪一次蓝（WebView 默认 tap highlight，.ns-game 一条 transparent 盖全部 canvas 蛋）。纯网页层热更、无 Kotlin 改动 |
