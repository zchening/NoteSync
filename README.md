...
| 版本 | 日期 | 摘要 |
|------|------|------|
| v7.3.3 | 2026-09-08 | 保存冲突再收敛（对抗审根修）：P0-1 提醒保存正文未装饰变化时 body=null 只传 rem（去陈旧底版 v+1 广播伪冲突主噪源）/ P0-4 干净设备 409 自动采纳他端真实变更不弹条（handleReminderConflict+applyRemoteBody）/ P0-5 历史恢复 force 强制落库+lastRestoreAt 3s 保护窗+poll 级2 恢复守卫不静默反转 / force 忙时保留（pendingForceResave）/ 系统通道挂起与预算耗尽 stashReminderDraft 防丢提醒；三子代理模块定向+功能核对6场景成立+两轮参谋+G裁判8/10，unit 380+e2e 66 全绿 |
