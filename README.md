...
## 🤖 用 AI 管理你的笔记（MCP）★ v6.3 新增 · 用法 v7.1.1 扩充

给 WorkBuddy（或任何支持 MCP 的 AI 助手）接上 NoteSync 后，你可以直接说人话。按想干的事分组，每句都能直接用：

**查正文**

- 「用 notesync 读一下我的笔记」
- 「看一下我的笔记全文」
- 「我的笔记里有没有提到体检？」（AI 读完正文帮你找）
- 「把我的笔记读成图片」（渲染成 PNG 给你；需要本机有 Playwright，见下方常见问题）
- 「我的笔记还在吗？同步到第几版了？」
...
- 「在『购物清单』后面补上：酱油两瓶」（按文字定位，插在它后面）
- 「把笔记里『旧地址：幸福路 12 号』那句删掉」
- AI 会先精确定位再动手，只动你说到的那段文字

**提醒**

- 「帮我在笔记里设个明天早上 9 点的提醒，事项写晨会」（会在正文末尾留一行「时间　事项」）
- 「这周五 18:30 提醒我收拾工位」
- 「下个月 1 号 18:50 提醒我交房租」
- 「笔记里有几条提醒？」（AI 读正文数时间条目）
- 「把明天早上 9 点那条提醒的事项改成周会」（同一时刻重设一遍，事项就更新了）
- 「笔记里都有哪些提醒？帮我列出来」
- 「取消明天早上 9 点那条提醒」（正文那行文字会留着，只是不再有下划线——和网页端取消一样）
- 「把已经触发过的提醒清理掉」

**图片**（v7.1.1 新增；v7.2.0 支持删图）

- 「把 D:\pics\cat.png 加到笔记末尾」
- 「在『今日膳食』后面插一张图：D:\pics\meal.jpg」
- 「把刚才加的那张图删掉」（AI 用添加时返回的图片地址精确删除；也可以直接给它 URL）
- 本机图片直传 Cloudinary 后以 `<img>` 写进加密正文，手机/网页都能看；单张 ≤8MB，不压缩

**搜索 / 备份**（★ v7.2.0 新增）

- 「帮我找找哪篇笔记提到『体检报告』」（全文检索，返回命中笔记与上下文摘要）
- 「把我的笔记全部备份一份」（导出 zip 到本机；默认明文可读，要求上云就说明用 raw 模式）
- 「从 D://backup//notesync-backup.zip 恢复『工作』这篇笔记」（先预览计划再确认写入）
...
| `note_edit` | 改内容：末尾追加 `append` / 按文字定位插入 `insert` / 删除 `delete` / 整篇替换为 HTML `replace_html`（★ v7.2.0） |
| `note_image` | 加图 / 删图（★ v7.1.1 / ★ v7.2.0 删图）：本机图片直传 Cloudinary 后插入正文 `<img>`；按图片地址删除 |
| `note_remind` | 提醒管理（★ v7.1.1 扩展）：设提醒 `add` / 列出 `list` / 取消 `cancel` / 清理过期 `clear` |
| `note_search` | 全文检索（★ v7.2.0）：中文按词索引 + 原文二次校验，返回命中笔记与摘要 |
| `note_export` | 一键备份 zip（★ v7.2.0）：`plain` 明文可读（md+html+图片附件）/ `raw` 纯密文可放心放云盘 |
| `note_import` | 从备份 zip 恢复（★ v7.2.0）：默认只预览计划，确认后才写入；版本冲突默认跳过 |

**`note_remind` 支持相对时间**（★ v7.1.0 新增）：`at` 除了 ISO 时间和 "YYYY-M-D H:MM" 写法，还支持口语相对时间——明天/后天/大后天/这周X/下周X/本月N日(号)/下个月N号，配合上午/下午/晚上时段自动换算，比如「明天上午10点10分」「这周五18:30」「下个月1号 18:50」都能直接设；过去时间（含 30 秒内）一律拒绝。

### MCP 能与不能（如实说）

| ✅ 能 | ❌ 不能 |
|------|--------|
| 查笔记状态（存在 / 第几版 / 口令能否解开） | 压缩图片（MCP 直传原图 ≤8MB；网页端粘贴才会自动压缩） |
| 读正文：纯文本 / 原始 HTML / 渲染成图片 | 删除正文里提醒留下的那行文字（取消提醒只删提醒本体，和网页端一致；文字可用 note_edit 删） |
| 逐句改文字：末尾追加 / 定位插入 / 删除（纯文本）；整篇替换为 HTML `replace_html`（★ v7.2.0） | 逐句写富文本（append/insert 只写纯文本，`**粗体**` 会存成字面星号；带格式内容用 `replace_html` 整篇给） |
| 加图 / 删图：本机图直传 Cloudinary 后插正文（★ v7.1.1），按图片地址删除（★ v7.2.0） | 用 note_edit 删跨结构的区间（链接、删除线等标签会被拒，提示缩小范围） |
| 提醒全套：设（回写正文行）/ 列出 / 取消 / 清理过期（★ v7.1.1） | |
| 全文检索（★ v7.2.0）：本地加密索引 + 原文二次校验，不存明文 | 检索结果实时性依赖笔记版本号增量（刚被另一台设备改写后下一次检索才会更新） |
| 备份/恢复 zip（★ v7.2.0）：plain 明文归档 / raw 密文可上云 | `plain` 导出是**明文**落盘（含提醒事项），注意保管位置；`.md` 仅给人看，恢复只认 `.html`/raw 密文，md 不反向还原富文本；附件图片只下载不改上传 |

几个硬限制：提醒事项**最长 20 字**（超出自动截断）；未来提醒**最多 10 条**；图片单张 **≤8MB** 不压缩直传；同刻重设提醒会更新提醒文案，但正文行会再追加一行（网页端同样如此，多余行可让 AI 删）；检索/备份遍历「注册表」（本机 `~/.notesync-mcp/registry.json`，用过的笔记名自动登记，也支持 `names` 显式指定）；导入默认 `preview` 只出计划，白名单外 HTML 标签报错拒绝、绝不静默剥离；多设备并发保存有版本冲突保护（版本落后写入被服务端 409 拒绝，网页端收冲突条人工拍板，MCP 端自动重读重试）。
...
- 多端同时编辑冲突时绝不自动覆盖：无实质差异的版本更新自动接受，真实冲突弹条二选一（保留本机 / 使用新版本）
- APK 冷启动自动进入上次打开的笔记（免输笔记名）；v7.3.1 起主动返回首页后的 120 秒内重开 App 会停在首页，之后重开恢复正常自动进入
- 限流数据存内存，服务重启清零
...
| 版本 | 日期 | 摘要 |
|------|------|------|
| v7.3.4 | 2026-09-08 | 保存冲突再收敛（块级三方合并，聚焦「A设备输入他端秒同步几乎从不冲突」）：真冲突弹条的最后一类「本机脏+远端真变」不再无脑二选一——以 lastHtml 为共同祖先，对 本机未保存输入(ours) × 远端快照(theirs) 做块级三方合并，两端各改不同块自动并集重发零冲突条，只有真同行冲突才回落弹条；hunk 模型显式建模两侧独立插入，本机插入+远端改同 gap 另一块时两不吞食，文首/文末/删除/插入边界逐块归位，整 run 多改无法逐位一律安全回落绝不错位吞内容；单块文档本机编辑远端未动自动采纳（去掉"整文档无保留"假的回落）；poll 级2 脏路径与 handleWriteConflict 409 双入口接 autoMergeSave，打字活跃期静默推迟免疫击键吵条；autoMergeSave 合并结果先落草稿防关页丢失、以远端权威 v 重发、组字/恢复保护窗内严禁自动合并、合并落地按偏移重建光标；比较器归一化缓存杜绝数百行便签合并冻结主线程。unit 405+e2e 66 全绿；对抗审追加修复：poll 级2 合并进来的远端提醒补推消费（防关页丢），详见 BUG_CHECKLIST v7.3.4 |
| v7.3.3 | 2026-09-08 | 保存冲突再收敛（对抗审根修，聚焦「A设备输入他端秒同步几乎从不冲突」）：P0-1 提醒保存正文未装饰变化时 body=null 只传 rem（去陈旧底版 v+1 广播伪冲突主噪源）/ P0-4 干净设备 409 自动采纳他端真实变更不弹条（handleReminderConflict+applyRemoteBody）/ P0-5 历史恢复 force 强制落库+lastRestoreAt 3s 保护窗+poll 级2 恢复守卫不静默反转 / force 忙时保留（pendingForceResave）/ 系统通道挂起与预算耗尽 stashReminderDraft 防丢提醒；App 端 logo 后常驻显示当前笔记名（长名截断防挤工具栏）+ e2e harness 拆解挂死根修；三子代理模块定向+功能核对6场景成立+两轮参谋+G裁判8/10，unit 380+e2e 66 全绿 |
| v7.3.2 | 2026-09-08 | 四连修：提醒浮卡3秒自动关闭（PC「弹窗未关闭」实为已添加卡常驻）/ 行首回车提醒行回跳（unwrapMark 拆包保 br）/ 光标消失（组字态复位+全路径焦点归还+防焦点抢夺）/ 新建提醒后光标自动换行；两轮子代理独立定位+对抗审核，unit 377+e2e 66 全绿 |
| v7.3.1 | 2026-09-08 | 返回首页有时点了没反应根修：根因X=APK冷启动自动跳转误伤主动返回首页（跳转标记带时间戳120s窗口+menuHome/maskClose先打标+笔记页pagehide兜底返回键）；根因Y=冲突浮卡/提醒卡盖住菜单项（#menuMask提到z90）；三子代理独立定位+三轮对抗审核，unit 360+e2e 26 全绿 |
| v7.3.0 | 2026-09-08 | 保存冲突频繁五根因修复：P0-1 解密比较自动接受 / P0-2 二次409收口 / P0-5 提醒系统解耦 / P2-4 保存互斥；HB1-6 补丁；三轮对抗审核 |
| v7.2.1 | 2026-09-08 | 提醒面板事项框暗文标注20字上限 |
| v7.2.0 | 2026-09-07 | MCP删图/整篇替换/检索/备份四工具；修覆盖丢数据、冲突误报、打字跳行、全角与悬停选时 |
...
<details>
<summary>完整更新详情（共 76 个版本）</summary>
