...
| 版本 | 日期 | 摘要 |
|------|------|------|
| v5.33 | 2026-09-04 | **借壳日间正式上线**：高风险环境（小米/QQ 浏览器 UA + 系统深色）自动启用——内部伪装夜间骗对手放行 + html `filter:invert(1)` + `#theme-shell` 预反色日间板，像素级复原日间模式；`notesync_darkshell` localStorage 开关（'1' 强制启用/'0' 强制禁用）；手动切换改走 `themeWantsDark` 语义状态变量（借壳时 body 恒为 dark，读 class 会判错方向）；系统深浅切换实时重判；MutationObserver 重排时保持 shell 压轴；借壳激活时不再弹深色环境提示条 |
| v5.32 | 2026-09-04 | **借壳日间方案**（用户真机观察直接破题）：夜间 + 滤镜 = 显示日间样式 → 证明对手放行深色页面。正式方案：内部伪装夜间（body.dark，对手放行）+ html 套 `filter:invert(1)` + `#theme-shell` 输出目标日间色的**预反色值**（滤镜一翻像素级精确复原日间配色），img/canvas 自带一层 invert 双重反转保持原样。`?themedi` 新增「借壳日间试一下」一键实验，两种浏览器夜间设置下验证。v5.31 提示条保留为借壳不适用时的兜底 |
| v5.31 | 2026-09-04 | 深色模式收尾：滤镜抵消真机实验无效 → 确认对手是「浅色检测型」强制反色（页面浅才反、深色放行，故夜间一直正常），页面侧无法阻止日间被改色。新增深色环境提示条（仅小米/QQ 浏览器 + 系统深色 + 页面日间时出现）：说清原因 + 一键切夜间 + 引导浏览器白名单，「我知道了」后本次会话不再打扰；`?themedi` 实验改为「白块A(固定FIXED) / 白块B(流内FLOW)」同色对照，判定对手是否跳过 fixed 层（决定全页容器化绕过是否可行）。主题策略不变 |
| v5.30 | 2026-09-04 | 深色模式取证二阶段：v5.29 真机取证确认对手是**渲染层强制反色**（computed 全部正确浅色但整页渲染深色，纯白探针变黑/纯黑变白，无视 only light；桌面 Chromium Force Dark 模拟尊重豁免、无法复现）。`?themedi` 升级为实验台：新增 RELATIVE/ABSOLUTE/FIXED 三组中灰定位探针（定位对手的跳过规则）+「滤镜抵消」开关（`filter:invert(1) hue-rotate(180deg)` 负负得正实验）。诊断工具增强，业务零改动 |
| v5.29 | 2026-09-04 | 修「系统深色模式下 QQ/小米浏览器切日间被强制反色」：① color-scheme 降级链（`only light` 需 Chromium 98+，旧内核会整条丢弃，改为探测后退回标准 `light` + 写回校验）；② 静态兜底由 `light dark` 改回 `light`（写 light dark 等于主动声明支持深色，反被反色）；③ `!important` 覆盖改挂 DOM 末位 + MutationObserver 顶回；④ 新增 `?themedi` 取证浮层。主题策略不变：默认按时间，手动切换仅当次会话有效 |
| v5.28 | 2026-08-28 | 扫码配对弹层两段文案换行优化（仅在逗号/冒号处加换行 + 精简冗余字），红字警示保持砖红 #C0453E，安全语义（等同口令等价警告、设备间）全保留；UI 微文案调整，服务端零改动，零知识不变 |
| v5.27 | 2026-08-27 | 新增 biji.xuyinji.com.cn 并行可信域：落地页提示文案域名动态适配当前访问域（biji/note/apex 各显示各的，不再写死 note）；`buildPairingUrl()` 新增 biji 显式直出分支（无中转），与 note 分支并列；biji 与 note 同库同应用（Caddyfile 新增反向代理块指向同一 127.0.0.1:8080 后端）；note 域中转逻辑不变 |
