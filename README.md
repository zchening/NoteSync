...
| 版本 | 日期 | 摘要 |
|------|------|------|
| v5.60 | 2026-09-06 | 十连修：新建笔记误报「发现另一台设备上的更新」根治（空密文落盐覆写并发端正文+版本号落后误判，服务端空值兜底）；菜单新增**修改口令**（先验旧口令，改后其他设备会提示「口令已变更」重新输入）；菜单新增**历史版本**（自动+手动快照 10 条，误删有得救，恢复不删历史）；右下角刷新按钮；PC/移动网页端扫码补 jsQR 兜底（桌面 Chrome/iOS 也能扫）；跨笔记提醒推送（A 设提醒切到 B 照常推，点通知直达 A）；APP 图标换暖纸白金衬线 N；菜单图标细线 SVG 化；页脚离线文案统一「· 最后同步：MM-DD hh:mm」 |
| v5.56 | 2026-09-06 | 真机验收六连修：「远端有更新」不再吞噬——本机有未保存改动时弹冲突条二选一、绝不静默覆盖（数据丢失级）；关标签/杀进程重开后提醒列表不再空（自动解锁补恢复）；他端新增/取消的提醒本端实时跟上（下划线有了）；APP 菜单新增「诊断」入口（bridge/闹钟/主文档缓存读数一眼看）；APK 离线三级兜底（联网→磁盘缓存→内置壳，断网必能开壳看缓存正文）；APK 版本号随版更新（v5.55 APK 误报 5.54 的治本） |
| v5.57 | 2026-09-06 | 真机验收九连修：点通知只进正文不再弹提醒面板；他端只改提醒时本端下划线也跟上；光标落时间必弹提醒卡（裸文本兜底+组字补触发）；冲突/草稿提示改扫码配对同款浮卡（排版不再挤爆，文案统一「本机 vs 服务器」）；口令框有「返回首页」出口；冷启动必回最后打开的笔记（自动解锁也记笔记名）；APK 主文档缓存真正落盘（装拦截器晚于首载的根治，离线兜底激活）；APP 菜单收纳日夜间/退出锁定且触控区加大；APP 新增「扫一扫」扫 PC 配对码直开笔记 |
| v5.58 | 2026-09-06 | 验收五连修：冲突卡加宽到 420 + 顶部强调条 + 按钮定稿「保留本机/使用新版本」；离线提示并入状态栏改「离线·同步时间：X」；页脚/菜单/☰ 三端全面加大；菜单三端统一（扫一扫/日夜间/退出锁定全平台进菜单、网页端自写扫码层）；**盐根治（数据级）**——空盐曾冲掉服务端盐致正确口令恒定解密失败：服务端空盐不覆写 + 前端兜底 + 解锁失败用缓存盐自愈回写 |
| v5.55 | 2026-09-06 | 推送通知栏彻底修通（桥接路径+传参形态双断点接上，前台不推/后台与关 App 必推）；APK 断网打开直接显示上次同步内容（原生主文档缓存兜底）；提醒时/分改滚轮；菜单去「菜单」标题、收藏夹收进二级、列表 ✕ 按钮退役（删收藏=进笔记取消）；输入中远端有更新时状态栏说真话 |
| v5.54 | 2026-09-06 | 页脚 ☰ 菜单（返回首页/收藏/收藏夹）替代双击返回；过期提醒彻底静默（删补弹 + REM_DONE 退役 + 原生 60 秒容差）；离线输口令直接解本地缓存进笔记（点过退出后离线不再死路）；补通知点击 JS 监听 |
| v5.53 | 2026-09-06 | APK 真机四连修：删 captureInput（简化 InputConnection 破坏 IME 组字链=汉字无法上屏+短语注入失效的根因）；首页输入框 250ms IME 兜底轮询；组字期间 input 监听器完全旁路 DOM 手术；edge-to-edge 退出（菜单栏离开挖孔区、触摸恢复）；返回键接 WebView 历史（回首页而非桌面）；开 webContentsDebuggingEnabled 便于真机取证 |
| v5.52 | 2026-09-06 | APK 改直连线上（server.url）：一次解决路径路由/CORS/QR 配对三类适配；砍掉热更新消除任意代码执行风险；补通知权限申请、修多提醒互相覆盖与落盘丢失；冷启动自动进上次笔记；新增断网兜底页；服务端修 XFF 伪造绕过锁定、/api/fail CSRF 刷锁、SSE 连接上限 |
| v5.51 | 2026-09-05 | APK 装小米：Capacitor 7 壳 + 自写 Kotlin RemPlugin + Keystore 加密 + AlarmManager 精确闹钟 + 开机重排 + 本地热更新 + GitHub Actions 云构建 release 签名；提醒关 App 也能推到通知栏 |
| v5.50 | 2026-09-05 | 断网实时感知：页面开着断网 1 秒内页脚变「离线」并浮出「离线 · 上次同步于 X」条，恢复联网自动回「已同步」并收条；离线打开提速 |
| v5.49 | 2026-09-05 | 新增离线阅读：断网打开也能看到上次同步的正文（底部浮出「离线 · 上次同步于 X」状态条）；旧离线草稿不再静默覆盖正文，一律弹冲突条确认 |
| v5.48 | 2026-09-05 | 修复小米手机 Chrome 长笔记末尾弹键盘时顶部菜单栏被顶出屏幕：声明键盘弹出压缩布局视口（Chrome 108+ 生效，顺带修复提醒面板被键盘遮挡） |
| v5.47 | 2026-09-05 | 提醒时间与事项分隔符由「·」改为全角空格；已添加提醒弹卡新增删除按钮；修复临近触发 30 秒内不弹卡；面板列表左对齐且按时间升序 |
| v5.46 | 2026-09-05 | 提醒下划线只包时间串；过期/已提醒的时间完全回归普通正文（不变灰无下划线）；已添加的时间悬停显示两行展示卡 |
| v5.45 | 2026-09-05 | 提醒四连改：面板时/分自建框打开即选中分钟；添加自动写入正文并带下划线；过期变灰零打扰；chip 更醒目 |
...
| v5.40 | 2026-09-05 | 提醒面板与到点卡片对齐配对弹窗风格，按钮更名添加提醒 |
| v5.39 | 2026-09-05 | 提醒面板极简+可设事项，过期时间明示，按钮防夜间黑块 |
