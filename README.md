...
| `note_image` | 加图片（★ v7.1.1）：本机图片直传 Cloudinary 后插入正文 `<img>` |
...
| 末尾追加、按文字定位插入、按内容或位置删除 | 写 Markdown / 富文本（只写纯文本；网址、手机号、时间由 App 自动识别渲染） |
| 加图片：本机图直传 Cloudinary 后插正文（★ v7.1.1） | 删带格式的整段结构（删除区间跨过链接、图片、删除线等会被拒绝，提示缩小范围） |
| 提醒全套：设（回写正文行）/ 列出 / 取消 / 清理过期（★ v7.1.1） | |

几个硬限制：提醒事项**最长 20 字**（超出自动截断）；未来提醒**最多 10 条**；图片单张 **≤8MB** 不压缩直传；同刻重设提醒会更新提醒文案，但正文行会再追加一行（网页端同样如此，多余行可让 AI 删）；操作都是「本机解密→改→加密→传回」，多设备冲突时和手动编辑一样有版本冲突保护（409 自动重试）。
