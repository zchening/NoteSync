...
## 🤖 用 AI 管理你的笔记（MCP）★ v6.3 新增 · 用法 v7.1.0 扩充
...
| ✅ 能 | ❌ 不能 |
|------|--------|
| 查笔记状态（存在 / 第几版 / 口令能否解开） | 插入图片（规划 v7.1.x 的 `note_image`，做好会更新这里） |
| 读正文：纯文本 / 原始 HTML / 渲染成图片 | 取消或删除提醒（去 App 或网页里删） |
| 末尾追加、按文字定位插入、按内容或位置删除 | 写 Markdown / 富文本（只写纯文本；网址、手机号、时间由 App 自动识别渲染） |
| 设提醒：ISO / "YYYY-M-D H:MM" / 中文相对时间 | 删带格式的整段结构（删除区间跨过链接、删除线等会被拒绝，提示缩小范围） |
| 同一时刻重设提醒 = 改事项 | |

几个硬限制：提醒事项**最长 20 字**（超出自动截断）；未来提醒**最多 10 条**；操作都是「本机解密→改→加密→传回」，多设备冲突时和手动编辑一样有版本冲突保护（409 自动重试）。
