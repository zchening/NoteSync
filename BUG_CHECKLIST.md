...
- **v5.32 借壳日间方案（用户观察破题）**: 用户真机实验发现「切夜间正常 → 点滤镜后页面显示日间样式」→ 对手放行深色页面 + 滤镜可翻色，两者组合 = 完整日间：内部伪装夜间（body.dark，对手放行）+ html `filter:invert(1)` + `#theme-shell` 预写目标日间色的反色值（像素级精确复原），img/canvas 自带 invert 双重反转保持原样。与对手行为解耦（夜间模式开/关两种设置下显示一致）。v5.32 已在 `?themedi` 提供「借壳日间试一下」一键验证，待真机确认后可固化为正式主题路径（applyTheme 分支：反色环境下「日间」= 内部夜间 + shell）
