...
| v8.0.2 | 用户续报：v8.0.1 起线齐但整列贴左、右侧大片空白失衡。终形=.mi-l 定宽 112px 标签列+justify-content:center——列宽恒定故图标/文字两列仍严格对齐，整组居中使留白两侧对称；#menuThemeLabel 同宽、#menuThemeIcon flex 化。menu_align 扩守三维（svg.x/文字 x/行高全等 ≤1px + scrollWidth 溢出≤2px——行高维堵定宽列换行盲区）；v801 重锚终形（11 行 mi-l 计数钉死）。教训：列表「齐」与「衡」要一次设计到位，分两版=闸外用户实测代偿。T2 漂移随版修两处（同族=并发饿死）：V547-2 裸等「秒≥30」改 :30–:42 窄带落位+前提自诊断（rAF 迟滞可把缓冲从 30s 吃成几秒→提醒先触发假红）；V72-1 的 1500ms 定值 sleep 条件化（已同步回位+localVer 4×300ms 稳定）——CLAUDE.md 红线18 墙钟测试纪律定档 |
