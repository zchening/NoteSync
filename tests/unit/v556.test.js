...
test('F8 MainActivity assets 兜底 + RemPlugin.cacheInfo + build.gradle/CI 版本号治本', () => {
  ...
  assert.ok(gradle.includes('versionCode 1000'), 'build.gradle versionCode 应随 APP_VERSION bump（标签不带版本注，v8.0.3 起根治第三连同族漂移）');
  assert.ok(gradle.includes('versionName "10.0.0"'), 'build.gradle versionName 应 bump 为 9.3.0');
