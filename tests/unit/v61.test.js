...
test('V61-14 版本升格：APP_VERSION 7.0.1 / BUILD_DATE / gradle 701+7.0.1 / README 条目 ≤40 汉字', () => {
  const src = readSrc();
  assert.ok(src.includes("const APP_VERSION = '9.3.9';"), 'APP_VERSION 应 9.3.0');
  assert.ok(src.includes("const BUILD_DATE = '2026-09-16';"), 'BUILD_DATE 应更新');
  const gradle = readRel('android/app/build.gradle');
  assert.ok(gradle.includes('versionCode 939') && gradle.includes('versionName "9.3.9"'), 'gradle 应 931/9.3.1');
