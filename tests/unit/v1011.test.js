...
test('V1011-T1f MCP assertName 行为：归一并返回小写名；非法名拒绝', () => {
...
test('V1011-T1g MCP 源码：resolveNames/工具调用点走 assertName 归一', () => {
  const s = MCP_SRC();
...
test('V1011-V1 版本三源字面 pin', () => {
  const s = SRC();
  assert.ok(s.includes("const APP_VERSION = '10.1.1';"), 'APP_VERSION');
  assert.ok(s.includes("const BUILD_DATE = '2026-09-25';"), 'BUILD_DATE');
  assert.ok(/versionCode 1011\b/.test(GRADLE()), 'gradle versionCode');
  assert.ok(/versionName "10\.1\.1"/.test(GRADLE()), 'gradle versionName');
  assert.ok(/version: '10\.1\.1'/.test(MCP_SRC()), 'MCP serverInfo version');
});
