...
// ── b. 关于弹窗 ───────────────────────────────────────────
test('V61B-b 关于弹窗：菜单入口打开，版本行含「Version 7.0.1」', async t => {
  const app = freshApp();
  t.after(() => app.dom.window.close());
  const { window, document } = app;
  ...
  assert.ok(document.getElementById('aboutVer').textContent.includes('Version 7.0.1'), '版本行应含「Version 7.0.1」');
});
...
test("V61B-c 彩蛋：800ms 内连点「关于NoteSync」4 次关关于弹 aboutMask 开 diagMask", async t => {
  ...
  assert.ok(txt.includes('7.0.1'), '诊断信息应含 7.0.1 版本行');
});
