...
// ── b. 关于弹窗 ───────────────────────────────────────────
test('V61B-b 关于弹窗：菜单入口打开，版本行含「Version 7.1.1」', async t => {
  const app = freshApp();
  t.after(() => app.dom.window.close());
  const { window, document } = app;
  ...
  assert.ok(document.getElementById('aboutVer').textContent.includes('Version 10.1.2'), '版本行应含「Version 10.0.4」');
});
...
test("V61B-c 彩蛋：800ms 内连点「关于NoteSync」4 次关关于弹 aboutMask 开 diagMask", async t => {
  ...
  assert.ok(txt.includes('10.1.2'), '诊断信息应含 9.3.1 版本行');
});
