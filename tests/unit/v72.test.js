...
// ═══════════ 源码断言 ═══════════
test('V72-S1 版本三处一致：index 8.0.5 / gradle 805 / MCP serverInfo 8.0.5', () => {
  assert.ok(SRC.includes("const APP_VERSION = '8.0.5';"), 'APP_VERSION 应 8.0.5');
  const gradle = fs.readFileSync(path.join(ROOT, 'android', 'app', 'build.gradle'), 'utf8');
  assert.ok(gradle.includes('versionCode 805') && gradle.includes('versionName "8.0.5"'), 'gradle 应 805/8.0.5');
  assert.ok(MCP_SRC.includes("serverInfo: { name: 'notesync', version: '8.0.5' }"), 'MCP serverInfo 应 8.0.5');
});
