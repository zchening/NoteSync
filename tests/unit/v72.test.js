...
// ═══════════ 源码断言 ═══════════
test('V72-S1 版本三处一致：index 8.0.4 / gradle 804 / MCP serverInfo 8.0.4', () => {
  assert.ok(SRC.includes("const APP_VERSION = '8.0.4';"), 'APP_VERSION 应 8.0.4');
  const gradle = fs.readFileSync(path.join(ROOT, 'android', 'app', 'build.gradle'), 'utf8');
  assert.ok(gradle.includes('versionCode 804') && gradle.includes('versionName "8.0.4"'), 'gradle 应 804/8.0.4');
  assert.ok(MCP_SRC.includes("serverInfo: { name: 'notesync', version: '8.0.4' }"), 'MCP serverInfo 应 8.0.4');
});
