...
// ═══════════ 源码断言 ═══════════
test('V72-S1 版本三处一致：index 7.7.0 / gradle 750 / MCP serverInfo 7.7.0', () => {
  assert.ok(SRC.includes("const APP_VERSION = '7.7.0';"), 'APP_VERSION 应 7.7.0');
  const gradle = fs.readFileSync(path.join(ROOT, 'android', 'app', 'build.gradle'), 'utf8');
  assert.ok(gradle.includes('versionCode 770') && gradle.includes('versionName "7.7.0"'), 'gradle 应 750/7.7.0');
  assert.ok(MCP_SRC.includes("serverInfo: { name: 'notesync', version: '7.7.0' }"), 'MCP serverInfo 应 7.7.0');
});
