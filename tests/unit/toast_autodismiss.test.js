...
// 只能整页刷新才消失（右下角 #refreshBtn 因只跑 poll 不碰 toast，清不掉）。
// 修：done 补「文案仍是它才清」的自动隐藏；开/关诊断、点刷新均 hideUploadStatus；忙时给轻提示。
'use strict';
...
test('右下角刷新：清提示 + 忙时轻提示不再纯静默', () => {
  assert.ok(src.includes('hideUploadStatus(); // v7.5.1：点刷新先清掉'), '点刷新应顺手 hideUploadStatus（与浏览器刷新观感对齐）');
