...
test('A2 居中必须配定宽标签列（无列居中=回潮到锯齿）', () => {
  assert.strictEqual((SRC.match(/<\/svg><span class="mi-l">/g) || []).length, 16, '16 行静态标签必须全包 mi-l（v8.1.0 前=11，打开链接+二级页三行 +4；v9.3.0 新增「桌宠」行 +1＝16；锚 </svg><span> 静态形态，不误计 A3 注入串里的 mi-l）');
